...
class Config:
    def __init__(self):
        ...
        self.is_multirms = False
        self.multirms_lock_file = self.stationID + ".lock"
...
def parseSystem(config, parser):
    ...
    if parser.has_option(section, "is_multirms"):
        config.is_multirms = parser.getboolean(section, "is_multirms")

    if parser.has_option(section, "multirms_lock_file"):
        config.multirms_lock_file = parser.get(section, "multirms_lock_file")
...
def parseCapture(config, parser):
    ...
    if parser.has_option(section, "use_CUDA"):
        config.use_cuda = parser.getboolean(section, "use_CUDA")

    if parser.has_option(section, "use_hevc"):
        config.use_hevc = parser.getboolean(section,"use_hevc")

    if parser.has_option(section, "timing_test"):
        config.timing_test = parser.getboolean(section, "timing_test")
    ...
    if parser.has_option(section, "ff_frames"):
        config.ff_frames = parser.getint(section, "ff_frames")
